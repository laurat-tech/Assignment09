...
#Laura Truong, 6.13.2020, Modified the rows in the read_data_from_file method to read the employee data formatted by rows
# ---------------------------------------------------------- #
...
class FileProcessor:
    ...
    def read_data_from_file(file_name: str):
        ...
        try:
            ...
            for line in file:
                row = line.split(",")
                list_of_rows.append(Emp(row[0], row[1], row[2].strip()))
            file.close()
        except Exception as e:
            print("There was a general error!")
            print(e, e.__doc__, type(e), sep='\n')
